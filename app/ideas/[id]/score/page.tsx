...
export default function ScoreIdeaPage() {
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col overflow-hidden bg-zinc-900 text-white">
      <div className="mx-auto w-full max-w-md">
        {/* Header */}
        <header className="flex items-center justify-between p-4 shrink-0">
          <Link href="/dashboard">
            <button className="flex h-10 w-10 items-center justify-center rounded-full text-zinc-400 hover:bg-zinc-800 transition-colors">
              <X className="h-6 w-6" />
            </button>
          </Link>
        ...
        <main className="flex-1 flex flex-col items-center justify-center px-6 py-12 min-h-[500px]">
          <div className="w-full max-w-sm flex flex-col gap-6 text-center">
            {/* Icon */}
            {IconComponent && (
              <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-blue-500/10 text-blue-500 mb-2">
            ...
            <div className="space-y-3">
              ...
              <p className="text-lg font-normal leading-relaxed text-zinc-300">
                {currentCriterion.description}
              </p>
            </div>

            {/* Range Labels */}
            <div className="mt-4 flex w-full justify-between px-2 text-xs font-medium text-zinc-500 uppercase tracking-wide">
            ...
            <div className="grid grid-cols-5 gap-2 w-full mt-2">
              {[1, 2, 3, 4, 5].map((score) => (
                <label key={score} className="group relative cursor-pointer">
                  ...
                  />
                  <div className="flex h-14 w-full items-center justify-center rounded bg-zinc-800 text-lg font-semibold text-zinc-400 transition-all duration-200 peer-checked:bg-blue-500 peer-checked:text-white peer-checked:shadow-lg peer-checked:shadow-blue-500/30 group-hover:bg-zinc-700">
                    {score}
                  </div>
                </label>
              ))}
        ...
        {/* Footer Actions */}
        <footer className="p-6 pb-8 shrink-0 flex flex-col gap-3 bg-gradient-to-t from-zinc-900 via-zinc-900 to-transparent">
          {!isLastStep ? (
            <Button
              onClick={handleNext}
              disabled={!canGoNext}
          ...
          {canGoBack && (
            <button
              onClick={handleBack}
              className="w-full h-10 text-sm font-medium text-zinc-400 hover:text-zinc-200 transition-colors"
            >
              Back to previous
            </button>
          )}
